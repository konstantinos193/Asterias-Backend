...
export class SettingsService {
  ...
  async upsertSettings(data: Partial<Settings>): Promise<Settings> {
    try {
      const allowedFields = [
        'maintenanceMode', 'emailNotifications', 'bookingConfirmations',
        'reminderNotifications', 'newBookingAlerts', 'lowInventoryAlerts',
        'reminderHours', 'checkInTime', 'checkOutTime', 'minAdvanceBooking',
        'maxAdvanceBooking', 'cancellationPolicy', 'overbookingAllowed',
        'currency', 'taxRate', 'municipalFee', 'environmentalTax',
        'automaticPricing', 'directBookingDiscount',
      ];
      ...
      this.settingsCache = updated as Settings;
      this.lastCacheUpdate = Date.now();
      return this.settingsCache;
    } catch (error) {
      this.logger.error('Error upserting settings:', error);
      throw error;
    }
  }
}
